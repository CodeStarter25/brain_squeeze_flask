    ...
    if request.method == "POST":
        ...
        try:
            ...
            # If 1 player, send data to database and close connection
                ...
                cursor.execute(
                    "INSERT INTO game_records (GameSelect, Letter, WinLossTie1, WinLossTie2, Username1, Username2, WordList1, WordList2, Score1, Score2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        session["gameselect"],
                        letter,
                        p1_status,
        ...
        data = {
            ...
            "p1": session.get("username1"),
            "p2": session.get("username2"),
...
def profile():
    ...
    if request.method == "POST":
        # Assign values
        email = (
            request.form.get("change_email", "").strip()
            if request.form.get("change_email")
            else None
        )
        password = (
            request.form.get("change_pass", "").strip()
...
def register():
    if request.method == "POST":
        ...
        if create == 0:
            ...
            flash(
                "Username not the correct length. Must be between 3 and 20 characters"
            )
            return redirect(url_for("register"))
        elif create == 3:
            ...
            flash(
                "Password not the correct length. Must be between 8 and 20 characters."
            )
            return redirect(url_for("register"))
        elif create == 8:
...
def results():
    ...
    data = {
        ...
        "p1": session.get("username1"),
        "p2": session.get("username2"),
        "p1_score": session.get("p1_score"),
        "p2_score": session.get("p2_score"),
        "p1_list": session.get("p1_list"),
        "p2_list": session.get("p2_list"),
    }
    all_data.append(data)
