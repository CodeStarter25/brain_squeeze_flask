...
def account_change(username, email, password, confirm):
    ...
    # Same email & Password change and matching
        ...
        try:
            ...
            logging.error(f"Error: could not update the password in database for user {username}")
        return 5
    ...
    # Email change && Password not change
        ...
        try:
            ...
            logging.error(f"Error: could not update the email in database for user {username}")
            return 5
    ...
    # Email & password change
        ...
        hash = generate_password_hash(password)
        try:
            cursor.execute("UPDATE users SET email = ?, hash = ? WHERE Username = ?", (email, hash, username))
            db.commit()
            return 4
        except sqlite3.Error as e:
            logging.error(f"Error: could not update the email and password in database for user {username}")
            return 5
...
def profile_history(username):
    ...
    try:
        cursor.execute("SELECT GameSelect, Letter, WinLossTie1, WinLossTie2, Username1, Username2, WordList1, WordList2, Score1, Score2, DateTime FROM game_records WHERE Username1 = ? or Username2 = ?", (username, username))
        record = cursor.fetchall() 
    except sqlite3.Error as e:
        logging.error(f"Error: could not pull profile history for user {username} or data does not exist")
    ...
    for row in record:
        data = {
            ...
            "Username1": row[4],
            "Username2": row[5],
            "WordList1": ", ".join([x if x.strip() else "/ " for x in re.findall(r"'(.*?)'", row[6])]) if row[6] else [],
            "WordList2": ", ".join([x if x.strip() else "/ " for x in re.findall(r"'(.*?)'", row[7])]) if row[7] else [],
            "Score1": row[8],
            "Score2": row[9],
            "DateTime": row[10],
        }
...
def scoring(p1_score, p2_score, p1_list, p2_list, letter):
    ...
    # Check if p1 score is less than 0 or more than 50 and set it to min (0) or max(50)
    ...
    # Check if p2 score is less than 0 or more than 50 and set it to min (0) or max(50)
    if p2_score < 0:
        p2_score = 0
    elif p2_score > 50:
        p2_score = 50
